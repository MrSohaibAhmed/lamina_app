                        ...
                        <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                            ...
                            <li className="nav-item topnav">
                                <div className="search-container d-flex">
                                    ...
                                    {showDropdown && (
                                        <div ref={dropdownRef} className="dropdown-menu position-absolute start-50 translate-middle-x w-30 show " style={{ borderRadius: "15px" }}>
                                            {/* Mapping through searchResults and rendering dropdown items */}
                                            {searchResults.map((pair, index) => (
                                                <div key={index} className="dropdown-item rounded" style={{ borderBottom: "1px solid #4d4b5a" }} onClick={() => handleClick(pair.pairAddress)}>
                                                    <div className='d-flex align-items-center p-3'>
                                                        <div className=''>
                                                            <img className='w-50 rounded mx-auto ' src={pair.info?.imageUrl} alt="Pair Image" />
                                                        </div>

                                                        <div>

                                                            <div> {pair.baseToken.name}</div>
                                                            <div><span style={{ fontSize: "12px", color: "white" }}>pair: </span>{pair.pairAddress}</div>
                                                            <div>{pair.dexId}</div>
