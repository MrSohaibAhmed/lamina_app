...
const TransactionTable = ({ address }) => {
    ...
    useEffect(() => {
        setTransaction(address).then((responseData) => (
                    ...
                    <tbody className="border-top-0">
                        {
                            data.map((item, index) => (
                                ...
                                <tr key={index} >
                                    ...
                                    <td className={item.side === "sell" ? "text-success" : "text-danger"}>{item.side}</td>
                                    <td  >{formatPrice(item.base.price)}</td>
                                    <td  >{formatTotalUSD(item.quote.uiAmount * item.base.uiAmount)}</td>
                                    {/* <td><img src={solIconImg} width="18px" alt="" /> 0.08718</td> */}
                                    <td  >{item.quote.amount / 1000}K</td>
                                    <td  ><img src={solIconImg} width="18px" alt="" /> {item.quote.uiAmount}</td>
