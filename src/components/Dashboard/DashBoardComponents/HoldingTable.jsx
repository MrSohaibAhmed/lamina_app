                    ...
                    <tbody className="border-top-0">
                        {
                            holdings.map((holding, index) =>(
                                holding.address !== 'So11111111111111111111111111111111111111111' ?( 
                                    <tr key={index} className=' align-items-center '>
                                        <td style={{verticalAlign:"middle"}}><img className='rounded-circle' height="40"  width= "40" src={holding.logoURI}/></td>
                                        <td style={{verticalAlign:"middle"}}>{holding.name}</td>
                                        <td style={{verticalAlign:"middle"}}>{holding.symbol}</td>
                                        {/* <td>SOL STREET BETS</td> */}
                                        {/* <td>{holding.address}</td> */}
                                        <td style={{verticalAlign:"middle"}}>{formatTxHash(holding.address)}</td>
                                        {/* <td> 3XUv67...7uU8</td> */}
                                        <td style={{verticalAlign:"middle"}}>${holding.priceUsd}</td>
                                        <td style={{verticalAlign:"middle"}}>${holding.balance}</td>
                                        <td style={{verticalAlign:"middle"}}>{holding.uiAmount/1000}K</td>
                                        <td style={{verticalAlign:"middle"}}><img src={solIconImg} width="18px" alt="" /> {holding.chainId}</td>
                                    </tr>):null
